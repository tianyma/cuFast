...
void cuFreeMemory() {
  ...
  // free(h_iscorner);
  // free(h_score);
  // CHECK_ERROR(cudaFreeHost(h_img));
  // CHECK_ERROR(cudaFreeHost(h_iscorner));
  // CHECK_ERROR(cudaFreeHost(h_score));
...
vector<corner> cuConerDetect(unsigned char *input, int width, int height,
  ...
  CHECK_ERROR(cudaMemsetAsync(d_score, 0, size_img_int, curstream));
  // cudaStreamSynchronize(streamCpt);
  // CHECK_ERROR(cudaMemsetAsync(d_iscorner, 0, size_img_int, curstream));
  CHECK_ERROR(cudaMemsetAsync(d_iscorner, 0, size_img_int, curstream));
...
;
  ...
  kernelCornerDetect<<<grid_size, block_size, size_sharedmem * 4, curstream>>>(
  ...
  CHECK_ERROR(
      cudaMemcpyAsync(h_iscorner, d_iscorner, size_img_int, cudaMemcpyDeviceToHost, curstream));
  CHECK_ERROR(
      cudaMemcpyAsync(h_score, d_score, size_img_int, cudaMemcpyDeviceToHost, curstream));
...
int main(int argc, char **argv) {
  ...
  cudaMallocHost(&h_img, srcimg.cols * srcimg.rows);
  cudaMallocHost(&h_iscorner, srcimg.cols * srcimg.rows * sizeof(int));
  cudaMallocHost(&h_score, srcimg.cols * srcimg.rows * sizeof(int));
  // cudaStream_t streamMem, streamCpt; // memory stream compute stream
  // cudaStream_t streamMem, streamCpt; // memory stream compute stream
  cudaStream_t stream[fn.size()];
  ...
  for (size_t i = 0; i < fn.size(); i++) {
    ...
    bool nonmax_suppression = 1;
      img = srcimgset[i];
      h_img = img.data;
      cuInitMemory(img.cols, img.rows);
      cuTransImage(h_img, stream[i]);
      vector<corner> cupoints;
      cupoints = cuConerDetect(h_img, img.cols, img.rows, threshold,
      stream[i]);
    //   for (size_t i = 0; i < cupoints.size(); i++) {
    //     // printf("%d\n", cupoints[i].score);
    //     circle(img, Point(cupoints[i].x, cupoints[i].y), 1,
    //            Scalar(0, 255, 255), 2);
    //   }
    //   imshow("img", img);
    //   waitKey(0);
